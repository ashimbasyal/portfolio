...
export const Terminal: React.FC<TerminalProps> = ({ isExpanded, onToggle }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const output = commands[command]
            ? commands[command]()
            : `Command not found: ${command}. Type 'help' to see the list of available commands.`;
            ...
            <AnimatePresence mode="wait">
                {!isExpanded ? (
                    <motion.button
                        ...
                        className="w-full h-full flex items-center justify-center space-x text-xs text-secondary-text dark:text-dark-highlight"
                    >
                        <span>Terminal</span>
